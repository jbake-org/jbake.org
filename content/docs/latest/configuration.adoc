...
:idprefix:
:url-defaultProperties: https://github.com/jbake-org/jbake/blob/master/jbake-core/src/main/resources/default.properties
...
=== Default hostname for server mode

This option defines the default hostname the server listen on
...
=== Default context path for server mode

This option defaults the default context path the server will use
